...
if (isset($_POST['log']) OR isset($_COOKIE['nom']))
{
	...
	if ($row != '')
	{
			...
			if($row['statut'] != 'ban' OR ($row['statut'] == 'ban' AND $row['fin_ban'] <= time()))
			{
				...
				else
				{
					...
					if(!array_key_exists('nom', $_SESSION))
					{
						...
						$requete = "INSERT INTO log_connexion VALUES('', ".$ID_base.", ".time().", '".$_SERVER['REMOTE_ADDR']."', 'Ok')";
						$db->query($requete);
					}
		...
		else
		{
			...
			{
				//Insertion dans les logs
				$requete = "INSERT INTO log_connexion VALUES('', ".$ID_base.", ".time().", '".$_SERVER['REMOTE_ADDR']."', 'Erreur mot de passe')";
				$db->query($requete);
			}
