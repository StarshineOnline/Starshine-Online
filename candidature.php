	...
	if(array_key_exists('type', $_GET))
	{
		...
		if(count($candidats) == 0)
		{
			...
			if($_GET['ministre_economie'] != '')
			{
				...
				{
          log_admin::log('debug', 'Ministre de l\'économie introuvable ('.count($economie).') : '.$_GET['ministre_economie'].' ('.$joueur->get_race().') -> '.sSQL($_GET['ministre_economie']), true);
			...
			if($_GET['ministre_militaire'] != '')
			{
				...
				{
          log_admin::log('debug', 'Ministre militaire introuvable ('.count($militaire).') : '.$_GET['ministre_militaire'].' ('.$joueur->get_race().') -> '.sSQL($_GET['ministre_militaire']), true);
