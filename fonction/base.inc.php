...
function recupmonstre($ID, $map_monstre = true)
{
	...
	if($map_monstre)
	{
		$requete = 'SELECT type, hp FROM map_monstre WHERE ID = '.$ID;
		$req = $db->query($requete);	
		$row = $db->read_array($req);
		$R_monstre['hp'] = $row['hp'];
	...
	while($test == false)
	{
		...
		elseif($row['type'] > 65 && $row['type'] < 75)
		{
		}
		else $test = true;
	}
...
function get_royaume_info($race_joueur, $royaume_id)
{
	...
	if($Roy_row['ID'] != 0)
	{
		...
		{
			case 127 :
				$Roy_row['taxe'] = floor($Roy_row['taxe'] / 4);
			break;
			case 0 :
				$Roy_row['taxe'] = ceil($Roy_row['taxe'] / 4);
			break;
			case 1 :
				$Roy_row['taxe'] = floor($Roy_row['taxe'] / 3);
			break;
			case 2 :
				$Roy_row['taxe'] = ceil($Roy_row['taxe'] / 3);
			break;
			case 3 :
				$Roy_row['taxe'] = floor($Roy_row['taxe'] / 2);
			break;
			case 4 :
				$Roy_row['taxe'] = ceil($Roy_row['taxe'] / 2);
			break;
			case 5 :
				$Roy_row['taxe'] = ceil($Roy_row['taxe'] / 1.5);
			break;
			case 6 :
				$Roy_row['taxe'] = ceil($Roy_row['taxe'] / 1);
			break;
			case 7 :
				$Roy_row['taxe'] = 0;
			break;
...
function lance_buff($type, $id, $effet, $effet2, $duree, $nom, $description, $type_cible, $debuff, $nb_buff, $grade)
{
	...
	if($db->num_rows == 0)
	{
		if($nb_buff < ($grade + 2))
		{
			$requete = "INSERT INTO ".$table."(`type`, `effet`, `effet2`, `fin`, `".$champ."`, `nom`, `description`, `debuff`) VALUES('".$type."', ".$effet.", ".$effet2.", ".(time()+$duree).", ".$id.", '".$nom."', '".addslashes($description)."', ".$debuff.")";
			$db->query($requete);
		}
		else
		{
...
function verif_mort($pourcent, $var)
{
	...
	if ($joueur['hp'] <= 0)
	{
		...
		if($var == 4 OR $var == 2)
		{
			...
			//Déplacement * 2
			$effet = 2;
			lance_buff('debuff_rez', $joueur['ID'], $effet, 0, $duree, 'Mal de résurrection', 'Mulitplie vos coûts de déplacement par 2', 'perso', 1, 0, 0);
		}
	}
}
...
function affiche_condition($action, $joueur)
{
	...
	elseif ($action[0] == '~')
	{
		...
		$requete = "SELECT nom, mp, comp_assoc FROM sort_combat WHERE id = ".$sort_sort;
		$req = $db->query($requete);
		$row = $db->read_assoc($req);
		$mpsort = round($row['mp'] * (1 - (($Trace[$joueur['race']]['affinite_'.$row['comp_assoc']] - 5) / 10)));
		$echo .= 'Lancer le sort <strong>'.$row['nom'].'</strong> <span class="small">('.$mpsort.' réserves)</span>';
	}
	elseif ($action[0] == '_')
	{
		$sort_sort = substr($action, 1, strlen($action));
		$requete = "SELECT nom, mp FROM comp_combat WHERE id = ".$sort_sort;
		$req = $db->query($requete);
		$row = $db->read_assoc($req);
		$echo .= 'Utiliser <strong>'.$row['nom'].'</strong> <span class="small">('.$row['mp'].' réserves)</span>';
	}
	elseif ($action[0] == '#')
	{
		...
		while ($j < $nb_arguments)
		{
			...
			if ($arg == '10')
			{
				$echo .= 'vous n\'êtes pas '.$parametre;
			}
			if ($arg == '11')
			{
				$echo .= 'l\'ennemi n\'est pas '.$parametre;
			}
			if ($arg == '12')
			{
				$echo .= 'vous êtes '.$parametre;
			}
			if ($arg == '13')
			{
				$echo .= 'l\'ennemi est '.$parametre;
			}
			if ($arg == '14')
			{
