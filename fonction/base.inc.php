...
function diff_sort($difficulte, $joueur, $type, $sortpa, $sortmp)
{
	...
	$pamp = 1 / $sortpa;//sqrt($sortpa * $sortmp);
	$total = ($facteur2 * $pamp * sqrt($comp / $difficulte));
	//echo $total.'<br />';
	return $total;
}
...
function affiche_ligne_journal($row)
{
	...
	switch($row['action'])
	{
		...
		break;
		case 'rez' :
			return '<li class="jbuff"><span class="small">['.$date.']</span> Vous avez récussité '.$row['passif'].' ('.$row['valeur'].'%).</li>';
		break;
		case 'rrez' :
			return '<li class="jbuff"><span class="small">['.$date.']</span> '.$row['passif'].' vous a récussité ('.$row['valeur'].'%).</li>';
		break;
		case 'teleport' :
			if ($row['valeur'] == 'jeu')
