...
function lance_sort($id, $acteur, &$effects)
{
  ...
  /* ~Debut */
	...
	$augmentation = augmentation_competence($row['comp_assoc'], $actif, (3.3 * sqrt(pow($actif[$row['comp_assoc']], 1.3) / ($row['difficulte'] / 4))));
	if ($augmentation[1] == 1)
	{
	...
	$augmentation = augmentation_competence('incantation', $actif, (2.3 * sqrt($actif['incantation'] / ($row['difficulte'] / 2))));
	if ($augmentation[1] == 1)
	{
