	...
	if ($attaque > $defense)
	{
		...
		if($actif['etat']['coup_bouclier'] > 0)
		{
			...
			if($atta > $defe)
			{
				...
				echo '&nbsp;&nbsp;Le coup de bouclier étourdit '.$passif['nom'].' !<br />';
			}
		}
		//sinon
		else
	...
	$PP = $effect->calcul_pp($actif, $passif, $PP);
      ...
      /* ~chance critique */
		...
		if($chance < $actif_chance_critique)
		{
			...
			{
				$passif['etat']['poison']['effet'] = $actif['etat']['fleche_poison']['effet'];
				$passif['etat']['poison']['duree'] = $actif['etat']['fleche_poison']['duree'];
			}
