      ...
      /*switch($sort->get_type())
      {
        ...
        case 'buff_critique' : case 'buff_evasion' : case 'buff_bouclier' : case 'buff_sacrifice' : case 'buff_inspiration' : case 'buff_force' : case 'buff_armure_glace' : case 'buff_barriere' : case 'buff_bouclier_sacre' : case 'buff_colere' : case 'buff_epine' : case 'buff_meditation' : case 'buff_rage_vampirique' : case 'buff_rapidite' : case 'buff_concentration' : case 'buff_furie_magique' : case 'buff_surpuissance' : case 'bouclier_feu' : case 'bouclier_terre' : case 'bouclier_eau' : case 'souffrance_extenuante' : case 'bulle_sanctuaire' : case 'bulle_dephasante' : case 'buff_contagion' :
          foreach($cibles as $cible)
          {
            ...
            if(lance_buff($sort->get_type(), $cible->get_id(), $sort->get_effet(), $sort->get_effet2(), $sort->get_duree(), $sort->get_nom(), description($sort->get_description(), $sort), $type_cible == 'joueur' ? 'perso' : 'monstre', 0, count($cible->get_buff_only()), $cible->get_grade()->get_nb_buff()))
            {
              $action = true;
          ...
          foreach($cibles as $cible)
          {
            ...
            if($cible->get_id() != $joueur->get_id())
            {
              ...
              if ($attaque > $defense)
              {
                ...
                {
                  echo 'Le sort '.$sort->get_nom().' a été lancé avec succès sur '.
                  $cible->get_nom().'<br />';
        ...
        case "guerison" :
          ...
          echo "<a href=\"\" onclick=\"return envoiInfo('sort.php?ID=".$_GET["ID"].$groupe.$lanceur_url.$groupe_href."', 'information')\">Utiliser de nouveau cette compétence</a>";
          break;
        ...
        case 'transfert_energie':
          ...
          foreach($cibles as $cible)
          {
            ...
            {
              //Insertion du 'soin' de groupe dans le journal de la cible
              $requete = "INSERT INTO journal(id_perso, action, actif, passif, time, valeur, valeur2, x, y) VALUES(".$cible->get_id().", 'rgbuff', '".$cible->get_nom()."', '".$joueur->get_nom()."', NOW(), '".$sort->get_nom()."', ".$gain.", ".$joueur->get_x().", ".$joueur->get_y().")";
              $db->query($requete);
            }
            else if($cible->get_id() != $joueur->get_id())
            {
              //Insertion du 'soin' dans le journal de la cible
              $requete = "INSERT INTO journal(id_perso, action, actif, passif, time, valeur, valeur2, x, y) VALUES(".$cible->get_id().", 'rbuff', '".$cible->get_nom()."', '".$joueur->get_nom()."', NOW(), '".$sort->get_nom()."', ".$gain.", ".$joueur->get_x().", ".$joueur->get_y().")";
              $db->query($requete);
            }
          }
          if ($lancement)
          ...
          '" onclick="return envoiInfo(this.href, \'information\')">Utiliser de nouveau ce sort</a>';
            ...
            if($groupe)
            {
              $requete = "INSERT INTO journal(id_perso, action, actif, passif, time, valeur, valeur2, x, y) VALUES(".$joueur->get_id().", 'rbuff', '".$joueur->get_nom()."', 'groupe', NOW(), '".$sort->get_nom()."', ".$gain_total.", ".$joueur->get_x().", ".$joueur->get_y().")";
              $db->query($requete);
            }
            else
            {
              $requete = "INSERT INTO journal(id_perso, action, actif, passif, time, valeur, valeur2, x, y) VALUES(".$joueur->get_id().", 'buff', '".$joueur->get_nom()."', '".$cible->get_nom()."', NOW(), '".$sort->get_nom()."', ".$gain_total.", ".$joueur->get_x().", ".$joueur->get_y().")";
              $db->query($requete);
            }
          }
          break;
        ...
        case 'liberation':
          ...
          foreach($cibles as $cible)
          {
            ...
            else
            {
              ...
              $db->query($requete);
              $gain = round($cible_perso->get_hp_maximum() * $sort->get_effet() / 100);
              if ($gain > $manque) $gain = $manque;
              $lancement = true;
              $gain_total += $gain;
              ...
              if($groupe)
              {
                ...
                $requete = "INSERT INTO journal(id_perso, action, actif, passif, time, valeur, valeur2, x, y) VALUES(".$cible->get_id().", 'rgsoin', '".$cible->get_nom()."', '".$joueur->get_nom()."', NOW(), ".$gain.", 0, ".$joueur->get_x().", ".$joueur->get_y().")";
                $db->query($requete);
              }
              else if($cible->get_id() != $joueur->get_id())
              {
                //Insertion du soin dans le journal
                $requete = "INSERT INTO journal(id_perso, action, actif, passif, time, valeur, valeur2, x, y) VALUES(".$cible->get_id().", 'rsoin', '".$cible->get_nom()."', '".$joueur->get_nom()."', NOW(), ".$gain.", 0, ".$joueur->get_x().", ".$joueur->get_y().")";
                $db->query($requete);
              }
            }
          }
          ...
          '" onclick="return envoiInfo(this.href, \'information\')">Utiliser de nouveau ce sort</a>';
            if($groupe)
            {
              $requete = "INSERT INTO journal(id_perso, action, actif, passif, time, valeur, valeur2, x, y) VALUES(".$joueur->get_id().", 'gsoin', '".$joueur->get_nom()."', 'groupe', NOW(), ".$gain_total.", 0, ".$joueur->get_x().", ".$joueur->get_y().")";
              $db->query($requete);
            }
            else
            {
              $requete = "INSERT INTO journal(id_perso, action, actif, passif, time, valeur, valeur2, x, y) VALUES(".$joueur->get_id().", 'soin', '".$joueur->get_nom()."', '".$cible->get_nom()."', NOW(), ".$gain_total.", 0, ".$joueur->get_x().", ".$joueur->get_y().")";
              $db->query($requete);
            }
          }
          break;
          ...
          foreach($cibles as $cible)
          {
            ...
            if($distance <= 7)
            {
              ...
              if ($attaque > $defense)
              {
                ...
                if(lance_buff($sort->get_type(), $cible->get_id(), $sort->get_effet(), $sort->get_effet2(), $duree, $sort->get_nom(), description($sort->get_description(), $sort), $type_cible == 'monstre' ? 'monstre' : 'perso', 1, 0, 0))
                {
                  echo 'Le sort '.$sort->get_nom().
                  ' a été lancé avec succès sur '.$cible->get_nom().'<br />';
                  // Insertion du debuff dans les journaux des 2 joueurs
                  if($type_cible != 'monstre')
                  {
                    $requete = "INSERT INTO journal VALUES(NULL,  ".$joueur->get_id().", 'debuff', '".$joueur->get_nom()."', '".$cible->get_nom()."', NOW(), '".$sort->get_nom()."', 0, ".$joueur->get_x().", ".$joueur->get_y().")";
