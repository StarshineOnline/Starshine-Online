...
else
{
	...
	if($_GET['tri'] == 'favoris')
	{
		$requete = "SELECT * FROM sort_jeu WHERE (cible = 2 OR cible = 1 OR cible = 8) AND type != 'rez' AND id IN (SELECT id_sort FROM sort_favoris WHERE id_perso = ".$joueur['ID'].") ORDER BY comp_assoc ASC, type ASC";
	}
	else
	{
		$requete = "SELECT * FROM sort_jeu WHERE (cible = 2 OR cible = 1 OR cible = 8) AND type != 'rez' ".$where." ORDER BY comp_assoc ASC, type ASC";
	}
	$req = $db->query($requete);
	$magie = '';
