...
if($joueur['groupe'] == $perso['groupe'])
{
	if ($perso['buff'] != NULL)
	{
		...
		{
			echo '<img src="image/buff/'.$buff['type'].'_p.png" alt="'.$buff['type'].'" ondblclick="if(confirm(\'Voulez vous supprimer '.$buff['nom'].' ?\')) envoiInfo(\'suppbuff.php?id='.$buff['id'].'\', \'perso\');" onmousemove="javascript:afficheInfo(\'info_p_'.$buff['type'].'\', \'block\', event); document.getElementById(\'info_p_'.$buff['type'].'\').style.zIndex = 2; document.getElementById(\'carte\').style.zIndex = 0;" onmouseout="javascript:afficheInfo(\'info_p_'.$buff['type'].'\', \'none\', event );" />
			<div class="infobox" id="info_p_'.$buff['type'].'">
		...
		{
			echo '<img src="image/buff/'.$buff['type'].'_p.png" alt="'.$buff['type'].'" onmousemove="javascript:afficheInfo(\'info_'.$perso['ID'].$buff['type'].'\', \'block\', event); document.getElementById(\'info_'.$perso['ID'].$buff['type'].'\').style.zIndex = 2; document.getElementById(\'carte\').style.zIndex = 0;" onmouseout="javascript:afficheInfo(\'info_'.$perso['ID'].$buff['type'].'\', \'none\', event );" />
			<div class="infobox" id="info_'.$perso['ID'].$buff['type'].'">
				<strong>'.$buff['nom'].'</strong><br />'.$buff['description'].'<br />Durée '.transform_sec_temp($buff['fin'] - time()).'
			</div>';
		}
	}
