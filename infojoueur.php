...
if ($perso->get_id() != $joueur->get_id())
{
	...
	if($perso->get_hp() > 0 AND (!$joueur->is_buff('repos_sage') AND !$joueur->is_buff('bloque_attaque') AND !$joueur->is_buff('dressage')) && $joueur->nb_pet() > 0) echo '<tr><td></td><td><a href="attaque.php?id_joueur='.$W_ID.'&amp;type=joueur&pet" onclick="return envoiInfo(this.href, \'information\')"> AC</a><span class="xsmall"> ('.($pa_attaque - $reduction_pa).' PA)</span></td></tr>';
}
