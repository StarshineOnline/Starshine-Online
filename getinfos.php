...
function cpyToAttr($node, $row, $attr) {
  if (is_array($attr)) {
    foreach ($attr as $tattr) {
      $s = iconv("ISO-8859-1", "UTF-8", $row[$tattr]);
      $node->setAttribute($tattr, $s);
    }
  }
  else {
    $s = iconv("ISO-8859-1", "UTF-8", $row[$attr]);
...
$requete_drapeaux = 'SELECT placement.x, placement.y, placement.type, placement.nom, placement.royaume, placement.debut_placement, placement.fin_placement, batiment.image, placement.hp FROM placement, batiment WHERE (((placement.x >= '.$xmin.') AND (placement.x <= '.$xmax.')) AND ((placement.y >= '.$ymin.') AND (placement.y <= '.$ymax.'))) AND batiment.id = placement.id_batiment ORDER BY placement.y ASC, placement.x ASC';
...
while($row = $db->read_array($req)) {
  ...
  if (!isset($_REQUEST['pc']) || $_REQUEST['pc']) {
	...
	$pc->setAttribute('mort', (($row_j[$i]['hp'] < 1) ? 'true' : 'false'));
	$square->appendChild($pc);
      }
    }
  }
