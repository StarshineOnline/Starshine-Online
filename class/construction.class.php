...
class construction extends entitenj_constr
{
  ...
  {
    global $db;
		$requete = 'SELECT x, y, b.image FROM '.static::get_table().' AS c INNER JOIN batiment AS b ON c.id_batiment = b.id WHERE x >= '.$x_min.' AND x <= '.$x_max.' AND y >= '.$y_min.' AND y <= '.$y_max.' AND '.$cond;
