...
/**
	...
	* @param int(11) effet attribut
	* @param int(11) effet2 attribut
	* @param int(11) id_monstre attribut
	...
	* @return none
	*/
	function __construct($id = 0, $type = '', $effet = '', $effet2 = '', $id_monstre = '', $fin = '', $duree = '', $nom = '', $description = '', $debuff = '', $supprimable = '')
	{
		global $db;
		//Verification nombre et du type d'argument pour construire l'etat adequat.
		if( (func_num_args() == 1) && is_numeric($id) )
		{
			$requeteSQL = $db->query("SELECT type, effet, effet2, id_monstre, fin, duree, nom, description, debuff, supprimable FROM buff_monstre WHERE id = ".$id);
			//Si le thread est dans la base, on le charge sinon on crée un thread vide.
			if( $db->num_rows($requeteSQL) > 0 )
			{
				list($this->type, $this->effet, $this->effet2, $this->id_monstre, $this->fin, $this->duree, $this->nom, $this->description, $this->debuff, $this->supprimable) = $db->read_array($requeteSQL);
			}
	...
	*/
	function sauver($force = false)
	{
		...
		if( $this->id > 0 )
		{
			...
			{
				if($force) $champs = 'type = "'.mysql_escape_string($this->type).'", effet = "'.mysql_escape_string($this->effet).'", effet2 = "'.mysql_escape_string($this->effet2).'", id_monstre = "'.mysql_escape_string($this->id_monstre).'", fin = "'.mysql_escape_string($this->fin).'", duree = "'.mysql_escape_string($this->duree).'", nom = "'.mysql_escape_string($this->nom).'", description = "'.mysql_escape_string($this->description).'", debuff = "'.mysql_escape_string($this->debuff).'", supprimable = "'.mysql_escape_string($this->supprimable).'"';
				else
				{
					$champs = '';
				...
				$requete = 'UPDATE buff_monstre SET ';
				$requete .= $champs;
		...
		{
			$requete = 'INSERT INTO buff_monstre (type, effet, effet2, id_monstre, fin, duree, nom, description, debuff, supprimable) VALUES(';
			$requete .= '"'.mysql_escape_string($this->type).'", "'.mysql_escape_string($this->effet).'", "'.mysql_escape_string($this->effet2).'", "'.mysql_escape_string($this->id_monstre).'", "'.mysql_escape_string($this->fin).'", "'.mysql_escape_string($this->duree).'", "'.mysql_escape_string($this->nom).'", "'.mysql_escape_string($this->description).'", "'.mysql_escape_string($this->debuff).'", "'.mysql_escape_string($this->supprimable).'")';
			$db->query($requete);
	...
	*/
	function supprimer()
	{
		...
		if( $this->id > 0 )
		{
			$requete = 'DELETE FROM buff_monstre WHERE id = '.$this->id;
			$db->query($requete);
		}
	}
	...
	*/
	static function create($champs, $valeurs, $ordre = 'id ASC', $keys = false, $where = false, $key_unique=false)
	{
		...
		$requete = "SELECT id, type, effet, effet2, id_monstre, fin, duree, nom, description, debuff, supprimable FROM buff_monstre WHERE ".$where." ORDER BY ".$ordre;
	...
	*/
	function __toString()
	{
		return 'id = '.$this->id.', type = '.$this->type.', effet = '.$this->effet.', effet2 = '.$this->effet2.', id_monstre = '.$this->id_monstre.', fin = '.$this->fin.', duree = '.$this->duree.', nom = '.$this->nom.', description = '.$this->description.', debuff = '.$this->debuff.', supprimable = '.$this->supprimable;
	}
	...
	* @access public
	* @param none
	* @return int(11) $effet valeur de l'attribut effet
	*/
	function get_effet()
	{
		return $this->effet;
	...
	* @access public
	* @param none
	* @return int(11) $id_monstre valeur de l'attribut id_monstre
	*/
	...
	* Modifie la valeur de l'attribut
	* @access public
	* @param int(11) $effet valeur de l'attribut
	* @return none
	*/
	...
	* Modifie la valeur de l'attribut
	* @access public
	* @param int(11) $id_monstre valeur de l'attribut
	* @return none
	*/
