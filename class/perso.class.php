...
class perso extends entite
{
  ...
  // @{
	 ...
	 *
	 * Le groupe du perso.
	 * Propriété avec "lazy loading" basique donc il faut toujours la get avec le getter (même dans le contexte de la classe perso)
     */
	private $groupe;
	
	private $id_groupe; 	///< Id du groupe du personnage.
	...
	*/
	...
	{
		if( $this->groupe instanceof undefined ){
	...
	/**
	* @param $groupe Un objet de la class groupe
	* @return none
	*/
	public function set_groupe($groupe)
	{
		$this->groupe = $groupe;
		if( $groupe instanceof groupe ){
			$this->set_id_groupe($groupe->get_id());
		}
	}
	...
	function set_id_groupe($id_groupe)
	{
		...
		$this->champs_modif['id_groupe'] = 'groupe';
		$this->set_groupe(new undefined());
	}
	...
	function prend_quete($id_quete)
	{
		...
		case 'royaume':
			...
			if( $row )
					$num_etape = $row[0];
		case 'groupe':
			$id_groupe = $this->get_id_groupe();
			if( $id_groupe )
			{
				$groupe = new groupe($id_groupe);
				/// @todo passer à l'objet
				$requete = 'SELECT id_etape FROM quete_perso AS qp INNER JOIN perso AS p ON qp.id_perso = p.id INNER JOIN quete_etape AS qe ON qe.id = qp.id_etape WHERE p.groupe = '.$this->id_groupe.' AND qp.id_quete = '.$quete->get_id().' ORDER BY qe.etape DESC LIMIT 1';
				$req = $db->query($requete);
				$row = $db->read_array($req);
				if( $row )
				...
				foreach($groupe->get_membre_joueur() as $membre)
				{
					...
					if( !$qp && $quete->a_requis($membre) && $etape )
					{
    ...
    if( $this->est_mort() )
    {
      ...
      if( $this->get_level() > 2 )
        $xp += 100 / ($this->get_level() - 2);
			...
			if($ennemi->get_id_groupe() > 0)
			{
				$groupe = new groupe($ennemi->get_id_groupe());
