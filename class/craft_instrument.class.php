...
class craft_instrument
{
	...
	/**	
		...
		*		-craft_instrument($array) qui associe les chasecrets de $array à l'objet.
	**/
	function __construct($id = 0, $nom = '', $description = '')
	{
		...
		{
			$requeteSQL = $db->query('SELECT nom, description FROM craft_instrument WHERE id = '.$id);
	...
	function sauver()
	{
		...
		{
			$requete = 'INSERT INTO craft_instrument (nom, description) VALUES(';
