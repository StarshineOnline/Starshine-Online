...
class messagerie_etat_message
{
	...
	**/
	function __construct($id_etat = 0, $id_message = 0, $etat = 'non_lu', $id_dest = 0, $groupe = 0, $echange = 0)
	{
		global $db;
		//Verification du nombre et du type d'argument pour construire l'etat adequat.
		if( (func_num_args() == 1) && is_numeric($id_etat) )
		{
			$requeteSQL = $db->query('SELECT id_message, etat, id_dest, groupe, echange FROM messagerie_etat WHERE id_etat = '.$id_etat);
			//Si le thread est dans la base, on le charge sinon on cr?e un thread vide.
			if( $db->num_rows($requeteSQL) > 0 )
			{
				list($this->id_message, $this->etat, $this->id_dest, $this->groupe, $this->echange) = $db->read_row($requeteSQL);
			}
			else
				$this->__construct();
		}
	...
	function sauver()
	{
		...
		if( $this->id_etat > 0 )
		{
			...
			$requete .= 'id_message = '.$this->id_message.', etat = "'.$this->etat.'", id_dest = '.$this->id_dest.', groupe = '.$this->groupe.', echange = '.$this->echange;
			$requete .= ' WHERE id_message_etat = '.$this->id_etat;
			$db->query($requete);
		}
		else
		{
			$requete = 'INSERT INTO messagerie_etat (id_message, etat, id_dest, groupe, echange) VALUES(';
			$requete .= $this->id_message.', "'.$this->etat.'", '.$this->id_dest.', '.$this->groupe.', '.$this->echange.')';
			$db->query($requete);
	...
	function __toString()
	{
		return $this->id_message.', '.$this->etat.', '.$this->id_dest.', '.$this->groupe.', '.$this->echange;
	}
}
?>