 ...
 * Elle ne doit pas être crée directement mais par l'ibtermédiare de l'objet gérant l'event
...
class event_participant extends table
{
	/**
		...
		-event($event, $id) qui récupère les informations de l'objet dans la base.
		-event($event, $vals) qui crée l'objet à partir d'information déjà récupèrées sans la base.
		-event($event, $perso, $equipe) qui crée un nouvel event.
   ...
   * - get_participant($event, $classe)                    renvoie un tableau contenant tous les participants
   * - get_participant($event, $classe, true)              renvoie tous les participants par équipe, sous forme de tableau associatif
   * - get_participant($event, $classe, $champ, $valeur)   renvoie un ou plusieurs participant suivant la valeur d'un champ précis
   *
