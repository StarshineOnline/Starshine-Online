...
?>
   <h2 class="ville_titre"><?php echo '<a href="javascript:envoiInfo(\'ville.php?poscase='.$W_case.'\', \'centre\')">';?><?php echo $R['nom'];?></a> - <?php echo '<a href="javascript:envoiInfo(\'enchanteur.php?poscase='.$W_case.'\', \'carte\')">';?> Enchanteur </a></h2>
	...
	if(isset($_GET['action']))
	{
		switch ($_GET['action'])
		{
			...
			case 'achat' :
				...
				if ($joueur['star'] >= $cout)
				{
					if(equip_objet('m'.$row['id'], $joueur))
					{
	...
	?>
		...
		<tr class="element trcolor<?php echo $couleur; ?>">
			...
			<td>
				...
				{
				?>
				<a href="javascript:envoiInfo('enchanteur.php?action=achat&amp;id=<?php echo $row['id']; ?>&amp;type=<?php echo $row['type']; ?>&amp;poscase=<?php echo $_GET['poscase'].$fort; ?>', 'carte')"><span class="achat">Achat</span></a>
				<?php
				}
				?>
			</td>
