...
<div id="liste_quete">
	<table width="99%">
		<tr>
			<td colspan="4" style="font-size : 0.9em; text-align : center;">Afficher : 
	...
	if($joueur['quete'] != '')
	{
		...
		if(count($quete_id) > 0)
		{
			...
			while($row = $db->read_array($req))
			{
				...
				echo '<tr>
				<td width="40%" onclick="javascript:envoiInfo(\'desc_quete.php?id_quete='.$row['id'].'&amp;quete_joueur='.$quest[$row['id']].'\', \'desc_quete\');">
					<span class="small" style="cursor : pointer;">'.$row['nom'].'
				</td>
				<td width="10%">
					Niv.'.$row['lvl_joueur'].'</span>
				</td>
				<td width="10%">
					'.$mode.$repetable.'</span>
				</td>
				<td width="20%">
					'.$total_fait.' / '.$total.'</span>
				</td>
				</tr>';
				?>
				<?php
