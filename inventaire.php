...
if( !$visu && $action )
{
  ...
  case 'slot_3':
	...
	case 'slot':
    $objet = objet_invent::factory( $obj );
		$dlg = $interf_princ->set_dialogue( new interf_dialogBS('Slottage de '.$objet->get_nom(), true) );
		$url = $G_url->copie('objet', $_GET['objet']);
		$dlg->add( new interf_lien('Slot de niveau 1 (1O PA, '.pourcent_reussite($perso->get_forge(), 10).'% de chances de réussite)', $url->get('action', 'slot_1')) );
		$dlg->add( new interf_bal_smpl('br') );
		$dlg->add( new interf_lien('Slot de niveau 2 (1O PA, '.pourcent_reussite($perso->get_forge(), 30).'% de chances de réussite)', $url->get('action', 'slot_2')) );
		$dlg->add( new interf_bal_smpl('br') );
		$dlg->add( new interf_lien('Slot de niveau 3 (1O PA, '.pourcent_reussite($perso->get_forge(), 100).'% de chances de réussite)', $url->get('action', 'slot_3')) );
		break;
	}
	//refresh_perso();
