...
if(!$visu AND isset($_GET['action']))
{
	switch($_GET['action'])
	{
			...
			switch($_GET['type'])
			{
				...
				case 'identification' :
					...
					while(!$fin AND $i < $count)
					{
						...
						$id_objet = $stack[0];
						if(substr($joueur['inventaire_slot'][$i], 0, 1) == 'h')
						{
							...
							//echo $id_objet;
							$requete = "SELECT * FROM gemme WHERE id = ".substr($id_objet, 2);
							$req = $db->query($requete);
							$row = $db->read_assoc($req);
							...
							if($player > $thing)
							{
								...
								$gemme = substr($joueur['inventaire_slot'][$i], 1);
				...
				case 'parchemin_tp' :
					$stack = explode('x', $joueur['inventaire_slot'][$_GET['key_slot']]);
					$id_objet = $stack[0];
					$id_objet_reel = substr($id_objet, 1);
		...
		case 'vente' :
			...
			else $id_objet = substr($id_objet, 1);
			switch ($categorie)
			{
				//Si c'est une arme
				case 'a' :
			...
			switch($loc['type'])
			{
				case 'arme' :
					if($joueur['inventaire']->$loc['loc'] != 'lock')
					{
						...
						echo '<img src="image/arme/arme/'.$row['image'].'.png" style="float : left;" />'.$Gtrad[$loc['loc']].'<br />'; 
				...
				case 'armure' :
					...
					$sqlQuery = $db->query($requete);
					$row = @$db->read_array($sqlQuery);
					echo '<img src="image/arme/armure'.$row['image'].'.png" style="float : left;" />'.$Gtrad[$loc['loc']].'<br />'; 
					echo '<strong>'.$row['nom'].'</strong>';
