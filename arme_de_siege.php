...
if($W_distance == 0)
{
	...
	while($row_bp = $db->read_assoc($req_bp))
	{
		if($row_bp['royaume'] != $Trace[$joueur->get_race()]['numrace'] && $row_bp['id'] != $arme->get_id())
		{
			...
			if($joueur->is_buff('debuff_rez')) $pa_attaque = 20;
			else $pa_attaque = 10;
			if($pat && $joueur->grade->get_rang() >= $row_b['bonus6'] && $joueur->get_pa() >= $pa_attaque) echo ' - <a href="attaque.php?type=siege&table=construction&id_arme_de_siege='.$arme->get_id().'&id_batiment='.$row_bp['id'].'" onclick="return envoiInfo(this.href, \'information\');">Attaquer avec l\'arme de siège ('.$pa_attaque.' PA)</a></li>';
		}
	}
	echo '</ul>';
