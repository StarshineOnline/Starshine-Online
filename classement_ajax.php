...
<?php
	...
	if($k < 0) $k = 0;
	$j = 26;
	$requete = "SELECT ID, nom, ".$tri.", level, race, classe, cache_stat, cache_classe FROM perso WHERE statut = 'actif' AND ".$where." ORDER BY ".$tri." DESC, nom ASC LIMIT $inf, $j";
		...
		<?php
			...
			while($row = $db->read_array($req))
			{
				if($z < 25)
				{
					...
					{
						$nom = $row['nom'];
						if($row['cache_classe'] == 1 AND (strtolower($row['nom']) != strtolower($_SESSION['nom']))) $row['classe'] = 'combattant';
