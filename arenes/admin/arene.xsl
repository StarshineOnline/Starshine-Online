...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template	match="/arene">
		<html>
            ...
            <xsl:call-template name="make_url">
              <xsl:with-param name="path" >css/interfacev2.css</xsl:with-param>
				...
				<link rel="stylesheet" type="text/css" >
					<xsl:attribute name="href">
						<xsl:call-template name="make_url">
							<xsl:with-param name="path" >arenes/admin/arenes.css</xsl:with-param>
						</xsl:call-template>
					</xsl:attribute>
				</link>
          ...
          <xsl:attribute name="src">
            <xsl:call-template name="make_url">
              <xsl:with-param name="path" >arenes/arene_poll.js</xsl:with-param>
						</xsl:call-template>
					</xsl:attribute>
				</script>
			</head>
...
-->
	...
	<xsl:template name="size_px">
		...
		select="(($size_cells*60)+40)" />px; height:<xsl:value-of
		select="(($size_cells*60)+40)" />px; </xsl:template>
	...
	<xsl:template name="journal">
		<xsl:for-each select="//journal/log">
				<li>
					<xsl:attribute name="class">
						<xsl:value-of select="@class"/>
					</xsl:attribute>
					<span class="small"><xsl:value-of select="@time"/><xsl:text>
					</xsl:text></span>
					<xsl:value-of select="@actif"/><xsl:text> </xsl:text>
					<xsl:value-of select="@action"/><xsl:text> </xsl:text>
					<xsl:value-of select="@passif"/><xsl:text> </xsl:text>
						...
						<xsl:if test="//joueur[@y=$row][@x=$col]">
							<div class="map_contenu" onmouseout="return nd();">
								...
								<xsl:attribute name="onmouseover">
									...
									/> - HP: <xsl:value-of select="@hp"/>/<xsl:value-of
									select="@hp_max"/> - MP: <xsl:value-of select="@mp"
									/>/<xsl:value-of select="@mp_max"/> - PA: <xsl:value-of
									select="@pa"/><![CDATA[</li>]]></xsl:for-each
