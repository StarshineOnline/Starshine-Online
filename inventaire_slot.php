	...
	foreach($joueur['inventaire_slot'] as $invent)
	{
		if($invent !== 0 AND $invent != '')
		{
			...
			if($check)
			{
				echo '
		<tr>
			<td onmousemove="afficheInfo(\'objet'.$objet_d['id'].'\', \'block\', event, \'centre\');" onmouseout="afficheInfo(\'objet'.$objet_d['id'].'\', \'none\', event, \'centre\');">
				'.$row['nom'];
			$modif_prix = 1;
			...
			//else echo ' X 1';
			echo '
				<table id="objet'.$objet_d['id'].'" style="display: none; z-index: 2; position: absolute; top: 250px; right: 150px; background-color:#ffffff; border: 1px solid #000000; font-size:12px; width: 150px; padding: 5px;">
					<tr>
						<td>
							'.description_objet($invent).'
						</td>
					</tr>
				</table>
			</td>
