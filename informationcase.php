...
if($num_rows > 0)
{
	...
	while($W_row = $db->read_array($W_query))
	{
		...
		if($W_row['x'] == $joueur["x"] AND $W_row['y'] == $joueur["y"] AND in_array('53', $quete_fini)) echo ' <a href="jeu2.php?donjon_id='.$W_row['id'].'">Entrer dans le donjon</a>';
		echo '<br /></div>';
	}
}
...
while($W_row = $db->read_array($W_query))
{
	...
	if($joueur['sort_jeu'] != '') echo ' <a href="javascript:envoiInfo(\'sort_monstre.php?poscase='.$W_case.'&amp;id_monstre='.$W_ID.'\', \'information\')"><img src="image/sort_hc_icone.png" title="Lancer un sort" alt="Lancer un sort" /></a>';
	echo '
		</td>
	</tr>';
}
