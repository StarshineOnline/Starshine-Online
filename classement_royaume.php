...
$tab = array();
$requete = "SELECT royaume.race as race_joueur, (SUM( cout ) * 200) AS tot FROM `construction` LEFT JOIN batiment ON construction.id_batiment = batiment.id LEFT JOIN royaume ON construction.royaume = royaume.id GROUP BY royaume ORDER BY tot DESC";
$req = $db->query($requete);
while($row = $db->read_assoc($req))
{
	$tableau[$row['race_joueur']] += $row['tot'];
...
$tab = array();
$requete = "SELECT royaume.race as race_joueur, (SUM( star_royaume ) * 100) AS tot FROM `quete_royaume` LEFT JOIN quete ON quete_royaume.id_quete = quete.id LEFT JOIN royaume ON quete_royaume.id_royaume = royaume.id GROUP BY id_royaume ORDER BY tot DESC";
$req = $db->query($requete);
while($row = $db->read_assoc($req))
{
	$tableau[$row['race_joueur']] += $row['tot'];
