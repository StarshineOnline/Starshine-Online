...
else
{
	...
	<?php
	$W_requete = 'SELECT mm.id, m.nom, mm.type, mm.hp, m.level FROM map_monstre mm, monstre m WHERE mm.type = m.id AND (mm.x = '.$joueur->get_x().') AND (mm.y = '.$joueur->get_y().') AND m.affiche != \'h\' AND m.level <= '.$joueur->max_dresse().' ORDER BY level ASC, nom ASC, id ASC';
