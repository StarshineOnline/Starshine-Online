		...
		while($row = $db->read_assoc($requete))
		{
			...
			if( $construction->get_hp() >= $batiment->get_hp() * $G_prct_vie_suppression )
			{
  			echo "<span style='display:block;width:30px;float:left;cursor:pointer;padding-left:4px;'>
  					<a onclick=\"if(confirm('Voulez-vous supprimer ce bâtiment : ".$construction->get_nom()." ?')) {return envoiInfo('construction.php?direction=suppr_construction&amp;id=".$construction->get_id()."', 'message_confirm');} else {return false;};\"><img src='../image/interface/croix_quitte.png' alt='suppression' title='Supprimer.'/></a>
  				</span>";
      }
      else
			{
